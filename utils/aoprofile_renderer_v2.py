...
def render_profile_png(profile: AOProfileData, avatar_bytes: bytes | None = None) -> bytes:
    ...
    footer_y = min(1540, max(1120, y + 80))
    ...
    final_height = min(CARD_SIZE[1], footer_y + 70)
    if final_height < CARD_SIZE[1]:
        canvas = canvas.crop((0, 0, CARD_SIZE[0], final_height))
    output = BytesIO()
    canvas.save(output, format="PNG", optimize=True)
    return output.getvalue()
