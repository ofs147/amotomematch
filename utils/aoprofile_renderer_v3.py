...
MAX_NOTE = 200
CARD_SIZE = (1080, 1620)
BUNDLED_FONT_PATH = Path(__file__).resolve().parents[1] / "assets" / "fonts" / "NotoSansSC-VF.ttf"
RENDERER_BUILD = "GRID3 FULLTEXT V7"
...
def _font(size: int, bold: bool = False) -> ImageFont.FreeTypeFont | ImageFont.ImageFont:
    ...
    for path in candidates:
        if path.exists():
            ...
            if path == bundled_font and hasattr(font, "set_variation_by_name"):
...
def _render_profile_png_legacy(profile: AOProfileData, avatar_bytes: bytes | None = None) -> bytes:
    ...
    y = _chips(draw, profile.favorite_games, y + 44, "#f8e8ef", 3) + 38
    ...
    y = _chips(draw, profile.oshi_names, y + 44, "#f7edf1", 3) + 38
...
def render_profile_png(profile: AOProfileData, avatar_bytes: bytes | None = None) -> bytes:
    """Render a three-column AOProfile PNG with all optional text preserved."""
    probe = ImageDraw.Draw(Image.new("RGB", (1, 1)))
    turn_off_lines = (
        _wrap_card_text(probe, profile.turn_offs, _font(27, True), 900, None)
        if profile.turn_offs else []
    )
    note_lines = (
        _wrap_card_text(probe, profile.note, _font(27, True), 900, None)
        if profile.note else []
    )

    # Calculate the complete content height before drawing.  The 1080px width
    # remains fixed while long optional text grows the PNG instead of being
    # clipped behind the footer.
    estimated_y = 382
    estimated_y += 68 + ((len(profile.favorite_games) + 2) // 3) * (82 + 12) + 12
    estimated_y += 68 + ((len(profile.xp_tags) + 2) // 3) * (58 + 12) + 12
    estimated_y += 68 + ((len(profile.oshi_names) + 2) // 3) * (68 + 12) + 10
    if turn_off_lines:
        estimated_y += 68 + len(turn_off_lines) * 37 + 8
    if note_lines:
        estimated_y += 68 + len(note_lines) * 37
    footer_y = max(1120, estimated_y + 80)
    canvas_height = max(CARD_SIZE[1], footer_y + 70)

    canvas = Image.new("RGB", (CARD_SIZE[0], canvas_height), "#fff8fc")
    draw = ImageDraw.Draw(canvas)
    start, end = (255, 247, 251), (239, 247, 254)
    for line_y in range(canvas_height):
        ratio = line_y / (canvas_height - 1)
        color = tuple(round(start[i] * (1 - ratio) + end[i] * ratio) for i in range(3))
        draw.line((0, line_y, CARD_SIZE[0], line_y), fill=color)
    draw.ellipse((770, -130, 1160, 260), fill="#e7f3fb")
    draw.ellipse((-170, canvas_height - 330, 210, canvas_height + 50), fill="#f7e5ee")
    draw.rounded_rectangle((38, 38, 1042, canvas_height - 38), radius=52, fill="#fffdfd", outline="#e5d4dd", width=3)
    ...
    y = _profile_grid(draw, profile.favorite_games, y, columns=3, fill="#f9e9f0", outline="#ebcbd8", font_size=25, row_height=82, max_lines=2) + 12
    ...
    y = _profile_grid(draw, profile.oshi_names, y, columns=3, fill="#f7eaf0", outline="#e9cbd7", font_size=27, row_height=68, max_lines=2) + 10

    if profile.turn_offs:
        y = _profile_section(draw, y, "雷点", "blue")
        for line in turn_off_lines:
